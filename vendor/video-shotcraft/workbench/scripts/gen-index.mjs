...
ensureLink(join(wb, 'demosrc'), '../demos');
mkdirSync(publicDir, { recursive: true });
ensureLink(join(publicDir, "clips"), relative(publicDir, join(repo, "assets/clips")));
ensureLink(join(publicDir, "cardpreviews"), relative(publicDir, join(repo, "gallery/media")));
ensureLink(join(publicDir, "sfxlib"), relative(publicDir, join(repo, "assets/audio/sfx")));
ensureLink(join(publicDir, "bgmlib"), relative(publicDir, join(repo, "assets/audio/bgm")));
// demo 纹理兜底：成片工程自己带 public/textures 时以工程为准（不往工程目录里塞东西）
{
  ...
  if (!texIsLink && !existsSync(join(tex, "live"))) ensureLink(join(tex, "live"), relative(tex, join(repo, "demos/_textures")));
}
...
const MATERIAL_REQUIRED = new Set();
const demosDir = join(wb, "demosrc");
const shotDurations = JSON.parse(readFileSync(join(wb, 'scripts/shot-durations.json'), 'utf8'));
const demos = [];
const walkDemos = (dir) => {
  for (const e of readdirSync(dir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (!durExport) {
      const budget = shotDurations[stem];
      if (!budget || !Number.isInteger(budget.frames) || budget.frames < 2) throw new Error(`Missing reviewed full-shot duration: ${stem}`);
      duration = budget.frames; durationSource = 'card';
    }
    const content = { text: [], media: [], colors: [] };
    const tree = ts.createSourceFile(p, src, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
    const collect = node => {
      if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node) || ts.isJsxText(node)) {
        const value = node.text.trim();
        if (/^#[0-9a-f]{3,8}$/i.test(value)) content.colors.push(value);
        else if (/^(textures|clips|images|media)\//.test(value)) content.media.push(value);
        else if (value.length > 1 && value.length < 220 && !/[{}#;/\\]/.test(value) && !/^(import|rgba?|translate|scale|linear|perspective)/.test(value) && (/[\u3400-\u9fff]/.test(value) || (/[a-zA-Z]/.test(value) && /\s/.test(value)))) content.text.push(value);
      }
      ts.forEachChild(node, collect);
    };
    collect(tree);
    for (const key of Object.keys(content)) content[key] = [...new Set(content[key])];
    demos.push({ content, stem, durExport, duration, durationSource, rel, category, slug });
  }
};
walkDemos(demosDir);
{
...
const aliases = JSON.parse(readFileSync(join(wb, 'scripts/gallery-aliases.json'), 'utf8'));
const meta = {};
for (const d of demos) {
  const card = cardBySlug.get(d.slug);
  const styles = card?.styles ?? [];
  let style = styles.find((s) => aliases[s.key] === d.stem || norm(s.key) === norm(d.stem));
...
writeFileSync(
  ...
  banner +
    ...
    `export type DemoMeta = { content?: {text: string[]; media: string[]; colors: string[]}; name: string; card: string; category: string; categoryKey: string; styleKey?: string; preview?: string; summary?: string };\n` +
...
// Product Video's public catalogue is generated from the same component registry.
const catalogue = demos.map(d => {
  const m = meta[d.stem];
  const source = readFileSync(join(demosDir, `${d.rel}.tsx`), 'utf8');
  let frames = d.duration || 150;
  if (d.durExport) {
    const value = new RegExp(`export const ${d.durExport}\\s*=\\s*([0-9. +*/()-]+)\\s*;`).exec(source)?.[1];
    if (value) frames = Math.round(Function(`return (${value})`)());
  }
  return { id: `demo:${d.stem}`, component: d.stem, card: d.slug, style: m.styleKey ?? d.stem, name: m.name, category: m.category, frames, fps: 30, source: `demos/${d.rel}.tsx`, recipe: `references/shots/${d.category}/${d.slug}.md` };
});
const missing = lib.cards.flatMap(c => c.styles.filter(s => !catalogue.some(d => d.style === s.key)).map(s => s.key));
if (missing.length) throw new Error(`Gallery variants without a component: ${missing.join(', ')}`);
writeFileSync(join(repo, 'motion-catalog.json'), JSON.stringify({ upstream: '5e71af35a2daee492dd3ea93e5e8903f32dcd13c', gallery: lib.stats, motions: catalogue, missing }, null, 2));
